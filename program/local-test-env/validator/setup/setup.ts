...
async function main() {
  if (fs.existsSync("./init-complete")) {
    console.log("\n");
    console.log(" >> Initialization seems to have completed already.                      << ");
    console.log(" >> Destroy the docker containers and recreate them to initialize again. << ");
    console.log("\n");
    return 0;
  }
  ...
  {
    console.log('Loading NEZ token program');
    const programPath = process.argv[3] as string;
    const programData: Buffer = fs.readFileSync(programPath);
    await BpfLoader.load(connection, adminKp, programKp, programData, BPF_LOADER_PROGRAM_ID);
  }
  ...
  fs.writeFileSync("./init-complete", "");
}
