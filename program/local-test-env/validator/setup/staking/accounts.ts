...
import { u8, u64 } from "./buffer_utils";

const PREFIX = "staking";

async function pda(program_id: PublicKey, seeds: (string | Buffer | PublicKey)[]): Promise<PublicKey> {
...
export async function epoch(program_id: PublicKey, index: number): Promise<PublicKey> {
	return await pda(
		program_id,
		[PREFIX, "EPOCH", u64(index)],
	)
}

export async function epoch_tier_winner(program_id: PublicKey, index: number, tier: number): Promise<PublicKey> {
	return await pda(
		program_id,
		[PREFIX, "EPOCH", u64(index), "TIER_WINNERS", u8(tier)],
	)
}
...
export async function deposit_attempt(program_id: PublicKey, owner: PublicKey): Promise<PublicKey> {
	return await pda(
		program_id,
		[PREFIX, "DEPOSIT_ATTEMPT", owner],

	)
}

export async function staking_ticket(program_id: PublicKey, epoch_index: number, owner: PublicKey): Promise<PublicKey> {
	return await pda(
		program_id,
		[PREFIX, "STAKING_TICKET", u64(epoch_index), owner],
	)
}
