...
use nezha_vrf_lib::error::{NezhaVrfError, SignatureType};
...
) -> Result<(), NezhaVrfError> {
    if !account.is_signer {
        return Err(NezhaVrfError::MissingSignature(SignatureType::SuperAdmin));
    }
    if account.key != &nezha_vrf_program_state.pubkeys.super_admin {
        return Err(NezhaVrfError::InvalidConstant(InvalidConstant::SuperAdminKey));
    }

    Ok(())
}

pub fn check_admin(account: &AccountInfo, nezha_vrf_program_state: &NezhaVrfProgramState) -> Result<(), NezhaVrfError> {
    if !account.is_signer {
        return Err(NezhaVrfError::MissingSignature(SignatureType::Admin));
    }
    if account.key != &nezha_vrf_program_state.pubkeys.admin {
        msg!("Error: Admin key mismatch");
        return Err(NezhaVrfError::InvalidConstant(InvalidConstant::AdminKey));
    }
