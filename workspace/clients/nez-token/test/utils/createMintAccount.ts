...
export class CreateMint extends Transaction {
  ...
  static async createMintAccount(ctx: TestContext, decimals: number, mint: Keypair = Keypair.generate()) {
    const mintRent = await ctx.connection.getMinimumBalanceForRentExemption(
      MintLayout.span,
      'confirmed',
    );
