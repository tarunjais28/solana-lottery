...
class Command:
    ...
    def _parse(self, argv_iter):
        ...
        for i, arg in argv_iter:
            if arg.startswith("-"):
                ...
                out = out | {
                    "command": arg,
                    **self.subcommands[arg]._parse(argv_iter),
                }
            else:
                if positional_index >= len(self.positionals):
                    if self.subcommands and not arg in self.subcommands:
                        raise InvalidOption(i, "/".join(self.subcommands.keys()), arg)
                    else:
    ...
    def _print_usage(self, name, indent_start=0, indent_incr=2):
        ...
        if self.switches:
            for arg in switches:
                s1 = "/".join(sorted(arg["switches"], key=lambda x: len(x)))
...
def _highlight_incorrect(argv, incorrect_index):
    ...
    return sep.join(
        filter(
            lambda x: x != "",
            (binary, correct_part_1, _red(wrong_part), correct_part_2),
        )
    )
